...
def define_simulator(sm):
    ...
    sm.register_component('PIPE_FRACTURE', circuit.Transducers(res=100000))
    ...
    sm.connect(sm.get_component('PIPE_1_S1').two, sm.get_component('PIPE_FRACTURE').one)
    sm.connect(sm.get_component('PIPE_FRACTURE').two, sm.get_component('WATER_IN_1').two)
